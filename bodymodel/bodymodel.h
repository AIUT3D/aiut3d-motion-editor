...
struct Effector
{
    ...
    // The effector must reach the target angle after the number of cycles specified by waitCycles is passed.
    double waitCycles;
    ...
    Effector(const double &minAngle, const double &maxAngle, const double &k1,
    		const double &k2, const double &k3, const double &errorTolerance)
    {
    ...
    void updateErrors()
    {
        ...
        currentError = (targetAngle - currentAngle);// / waitCycles;
    ...
    // AI lab Hoseini 95-09-24 start
    void setWaitCycles(const double &cycles)
    {
        waitCycles = cycles;
    }
    // AI lab Hoseini 95-09-24 end
    ...
    /* AI lab Hoseini 95-09-24 start
     * The method "void update(const double &angle)" is not used at all in the code. So I removed it :(
     * currentAngle is set somewhere else using BodyModel::setCurrentAngle().
     * AI lab Hoseini 95-09-24 end
     */
...
public:
    ...
    inline std::vector<Effector> GetEffector()
    {
        return effector;
    }
    ...
    // AI lab Hoseini 95-09-24 start
    inline void setTargetWaitCycles(const int &EffectorID, const double &cycles)
    {
        effector[EffectorID].setWaitCycles(cycles);
    }
    // AI lab Hoseini 95-09-24 end
